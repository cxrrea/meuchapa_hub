...
import { Search, ArrowLeft, Archive } from 'lucide-react';

export default function ArchivedTickets() {
  const navigate = useNavigate();
  const { profile, loading: authLoading } = useAuth();
  const { tickets, loading } = useArchivedTickets();
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-2xl font-bold text-foreground">Chamados Arquivados</h1>
            </div>
            <p className="text-muted-foreground ml-10">
              Visualize chamados que foram arquivados
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Buscar chamados arquivados..."
              value={search}
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              {search || statusFilter !== 'all' || priorityFilter !== 'all'
                ...
                : 'Não há chamados arquivados no momento'}
            </p>
          </div>
        ) : (
